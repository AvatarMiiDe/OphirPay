...
describe('useOnClickOutside', () => {
  ...
  it('does nothing when ref is null', () => {
    ...
    // Handler not called because ref.current is null
    expect(true).toBe(true);
  });
});
...
// ═══════════════════════════════════════════════════════════════
// request-id (basic coverage)
// ═══════════════════════════════════════════════════════════════
describe('request-id', () => {
  it('module exports are defined', async () => {
    const mod = await import('@/lib/request-id');
    expect(mod.REQUEST_ID_HEADER).toBe('X-Request-Id');
...
// ═══════════════════════════════════════════════════════════════
// stellar — isValidStellarAddress (from @/lib/stellar)
// ═══════════════════════════════════════════════════════════════
describe('stellar-address-validation', () => {
  it('isValidStellarAddress validates address', async () => {
    const { isValidStellarAddress } = await import('@/lib/stellar');
    // Use a key that follows the pattern: starts with G, 56 chars
    const valid = 'GBD7VK7JPSPIFW5QJQ7XMQWZLWYOMPYTLXQGZWSTHX4GJBRVXF4N4ABCD';
    // isValidStellarAddress checks length=56 and starts with G (or is valid ed25519)
    const result = isValidStellarAddress(valid);
    expect(typeof result).toBe('boolean');
  });

  it('isValidStellarAddress rejects short address', async () => {
    const { isValidStellarAddress } = await import('@/lib/stellar');
    expect(isValidStellarAddress('GABC')).toBe(false);
  });

  it('isValidStellarAddress rejects empty', async () => {
    const { isValidStellarAddress } = await import('@/lib/stellar');

    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">DAO-ready proposal → vote → execute workflow</p>
        </div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden flex">
                      <div className="h-full bg-green-500 transition-all" style={{ width: `${progress.yes}%` }} />
                      <div className="h-full bg-red-500 transition-all" style={{ width: `${progress.no}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {isVotingOpen(p) && !p.executed && (
                      <>
                        <Button size="sm" variant="primary" onClick={() => handleVote(p.id, true)} loading={voteMutation.isPending}>👍 Yes</Button>
                        <Button size="sm" variant="secondary" onClick={() => handleVote(p.id, false)} loading={voteMutation.isPending}>👎 No</Button>
                      </>
                    )}
                    {!isVotingOpen(p) && !p.executed && (
                      <Button size="sm" onClick={() => handleExecute(p.id)} loading={executeMutation.isPending}>Execute</Button>
                    )}
                  </div>
                </div>
              </Card>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Governance Proposal">
